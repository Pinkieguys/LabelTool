"""
This library is a sub-library for SPAM (Software for Processing and Analysis of Materials), 
primarily used for segmentation refinement, including over-segmentation and under-segmentation detection and repair.
Originally designed for Linux, it has been modified for Windows compatibility.
"""
import concurrent
import numpy as np
...
def localDetection_modiefied(subVolGrey,localThreshold,radiusThresh=None,subset=None, verbose=False):
    ...
    if localThreshold < 0:
        #自适应
        try:
            strc = np.ones((-localThreshold,-localThreshold,-localThreshold))
...
def computeAngle(args):
    ...
    return minIndex
def plot_spherical_histogram(orientations, subDiv=3, reflection=True, maxVal=None, verbose=True, color=None, viewAnglesDeg=[25, 45], title=None, saveFigPath=None):
...
def fix_over_segmentation(lab, targetLabels, verbose=True, imShowProgress=False):
    """
    ...
    -----------
        lab : 3D numpy array
            Labelled image
        targetLabels : list of tuples
            List of label pairs to merge, e.g. [(1,2),(3,4)]
        verbose : bool
            Show progress bar
        imShowProgress : bool
            Show visualization of merging process
    ...
    finishedLabels = set()  # Use set for O(1) lookup
    processedPairs = 0

    # Initialize progress bar
    if verbose:
        import progressbar
        widgets = [progressbar.FormatLabel('Merging labels: '), ' ',
                   progressbar.Bar(), ' ', progressbar.AdaptiveETA()]
        pbar = progressbar.ProgressBar(widgets=widgets, maxval=len(targetLabels))
        pbar.start()

    # Main loop
    for labelA, labelB in targetLabels:
        # Skip if either label is already processed
        if labelA in finishedLabels or labelB in finishedLabels:
            if verbose:
                processedPairs += 1
                pbar.update(processedPairs)
            continue

        # Check if labels still exist in the image
        if not (numpy.any(lab == labelA) and numpy.any(lab == labelB)):
            if verbose:
                processedPairs += 1
                pbar.update(processedPairs)
            continue

        # Mark labels as processed
        finishedLabels.update([labelA, labelB])

        # Get the subset of the two labels
        try:
            subset = spam.label.fetchTwoGrains(lab, [labelA, labelB])
        except:
            # Skip if fetchTwoGrains fails
            if verbose:
                processedPairs += 1
                pbar.update(processedPairs)
            continue
        ...
        if imShowProgress:
            ...
            subVolLabNorm = subset['subVolLab'].copy()
            subVolLabNorm = numpy.where(subVolLabNorm == labelA, 1, subVolLabNorm)
            subVolLabNorm = numpy.where(subVolLabNorm == labelB, 2, subVolLabNorm)

            # Create merged version for display
            subVolLabNew = numpy.where(subset['subVolLab'] == labelB, labelA, subset['subVolLab'])

            # Plot comparison
            fig, axes = plt.subplots(3, 2, figsize=(10, 12))
            fig.suptitle(f'Merging Label {labelB} into Label {labelA}', fontsize=14)

            # Get middle slices
            mid_z = subset['subVolLab'].shape[0] // 2
            mid_y = subset['subVolLab'].shape[1] // 2
            mid_x = subset['subVolLab'].shape[2] // 2

            # XY plane
            axes[0, 0].imshow(subVolLabNorm[mid_z, :, :], cmap="cubehelix")
            axes[0, 0].set_title('Before - XY plane')
            axes[0, 1].imshow(subVolLabNew[mid_z, :, :], cmap="cubehelix")
            axes[0, 1].set_title('After - XY plane')

            # XZ plane
            axes[1, 0].imshow(subVolLabNorm[:, mid_y, :], cmap="cubehelix")
            axes[1, 0].set_title('Before - XZ plane')
            axes[1, 1].imshow(subVolLabNew[:, mid_y, :], cmap="cubehelix")
            axes[1, 1].set_title('After - XZ plane')

            # YZ plane
            axes[2, 0].imshow(subVolLabNorm[:, :, mid_x], cmap="cubehelix")
            axes[2, 0].set_title('Before - YZ plane')
            axes[2, 1].imshow(subVolLabNew[:, :, mid_x], cmap="cubehelix")
            axes[2, 1].set_title('After - YZ plane')

            plt.tight_layout()
            plt.show()

        # Perform the actual merging - directly modify lab
        # This avoids the cumulative addition problem
        lab = numpy.where(lab == labelB, labelA, lab)
    ...
    # Update the labels to make them sequential


    if verbose:
        pbar.finish()
        print(f"Merged {processedPairs} label pairs successfully.")
    ...
    for label in labelsList:
        ...
        if label == 0:
            ...
            if (dilOnly.shape == labSlice.shape):
                ...
                counts = numpy.unique(intersection, return_czounts=True)
...
def detect_over_segmentation(lab):
    # Get the labels
...
def convexVolume_modiefied(lab, boundingBoxes=None, centresOfMass=None, volumes=None, nProcesses=nProcessesDefault, verbose=True):
    ...
    if verbose:
        pbar.finish()
    return convexVolume
def detect_under_segmentation(lab, nProcesses=nProcessesDefault, verbose=True):
    # Compute the volume
    vol = spam.label.volumes(lab)
    print(nProcesses)
...
def getLabel_modiefied(labelledVolume, label, boundingBoxes=None, imGrey=None, padding=0):
    output = {}

    startZ = max(boundingBoxes[label, 0] - padding, 0)
    stopZ = min(boundingBoxes[label, 1] + padding, labelledVolume.shape[0] - 1)
    startY = max(boundingBoxes[label, 2] - padding, 0)
    stopY = min(boundingBoxes[label, 3] + padding, labelledVolume.shape[1] - 1)
    startX = max(boundingBoxes[label, 4] - padding, 0)
    stopX = min(boundingBoxes[label, 5] + padding, labelledVolume.shape[2] - 1)
    ...
    subVolLab = labelledVolume[
                startZ:stopZ + 1,
                startY:stopY + 1,
                startX:stopX + 1
                ]

    subVolLab = numpy.where(subVolLab == label, 1, 0)
    ...
    if imGrey is not None:
        ...
        output['harmard'] = numpy.where(subVolLab == 1, greySubVol, 0)

    output['subvol'] = subVolLab

    return output


def fix_under_segmentation_simplified(lab, imGrey, targetLabels, underSegCoeff, boundingBoxes=None,
                                             centresOfMass=None, imShowProgress=False, verbose=True,
    ...
    for label in targetLabels:
        # Get the subset
        labelData = getLabel_modiefied(lab, label, boundingBoxes, imGrey, padding=2)
        # 更改
        try:
            greySubset = labelData['harmard']
        except:
        ...
        thresh = 0.2
        passBool = 'Hover'
        seeds = spam.label.watershed(greySubset >= thresh)
        print(f"{label}:{thresh},{numpy.max(seeds)}")
        # Do we have seeds?
        while numpy.max(seeds) != 2:
            if thresh > 0.99:
                passBool = 'FuckOFF'
                print(f"{label}:FuckOFF")
                break
            thresh += 0.01
            seeds = spam.label.watershed(greySubset >= thresh)
            print(f"{label}:{thresh},{numpy.max(seeds)}")
        ...
        if passBool != 'FuckOFF':
            ...
            if not disableCoeffCheck:
                ...
                if len(res) > 2:
                    ...
                    if not all(map(lambda x: x < underSegCoeff[label], res[1:])):
                        shouldAccept = False
                    print("pass oversegCoeff test")
                else:
            ...
            if shouldAccept:
                ...
                slices = labelData['slices']

                lab[slices[0], slices[1], slices[2]] += imLabSubsetNew.astype(np.uint32)
            ...
            # We shouldn't change this label
            passBool = 'Decline'
        if imShowProgress and passBool != "FuckOFF" and passBool != "Decline":
    ...
    if verbose:
        pbar.finish()
        print('\n spam.label.fixUndersegmentation(): From {} target labels, {} were modified'.format(len(targetLabels),
                                                                                                     successCounter))
    return lab


def fix_under_segmentation(lab, imGrey, targetLabels, underSegCoeff, boundingBoxes=None, centresOfMass=None, imShowProgress=False, verbose=True, disableCoeffCheck=False):
    """
    ...
    for label in targetLabels:
        # Get the subset
        labelData = spam.label.getLabel(lab, label, margin = 1, boundingBoxes=boundingBoxes, centresOfMass=centresOfMass, extractCube=True, imGrey=imGrey)
        #更改
        try:
            greySubset = labelData['harmard']
        except:
        ...
        if passBool != 'FuckOFF':
            ...
            if not disableCoeffCheck:
                ...
                labelData['sliceCube'][2].start:labelData['sliceCube'][2].stop] = imLabSubsetNew
                """
                # 基于边界框和 imLabSubsetNew 的实际大小计算切片
                z_start = max(0, labelData['boundingBoxCube'][0])
                z_end = min(lab.shape[0], z_start + imLabSubsetNew.shape[0])
                y_start = max(0, labelData['boundingBoxCube'][2])
                y_end = min(lab.shape[1], y_start + imLabSubsetNew.shape[1])
                x_start = max(0, labelData['boundingBoxCube'][4])
                x_end = min(lab.shape[2], x_start + imLabSubsetNew.shape[2])

                # 计算需要从 imLabSubsetNew 中提取的部分
                z_len = z_end - z_start
                y_len = y_end - y_start
                x_len = x_end - x_start

                # 分配相应部分
                labelDummyUnit[z_start:z_end, y_start:y_end, x_start:x_end] = imLabSubsetNew[:z_len, :y_len, :x_len]
                # Add the grains
                labelDummy = labelDummy + labelDummyUnit


        if imShowProgress and passBool != "FuckOFF":
            # Enter graphical mode
            # Change the labels to show different colourss
            fig=plt.figure()
            # Plot
