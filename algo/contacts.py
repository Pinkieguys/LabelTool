...
def _contactNormals_new(dataSet):
    """
    ...
    # 对特征值进行排序，获取排序后的索引
    sorted_indices = numpy.argsort(eigVal)

    # 检查特征值是否为0（考虑浮点数精度）
    is_zero = numpy.isclose(eigVal, 0)

    # 如果所有特征值都是0，抛出异常
    if numpy.all(is_zero):
        raise ValueError("All eigenvalues are zero, cannot determine contact normal")

    # 按照优先级选择特征向量
    if is_zero[sorted_indices[0]]:  # 最小特征值为0
        if is_zero[sorted_indices[1]]:  # 第二小特征值也为0
            # 使用最大特征值对应的特征向量
            minEV = eigVec[:, sorted_indices[2]]
        else:
            # 使用第二小特征值对应的特征向量
            minEV = eigVec[:, sorted_indices[1]]
    else:
        # 使用最小特征值对应的特征向量
        minEV = eigVec[:, sorted_indices[0]]

    # 确保法向量的 z 分量为正
    contactNormal = numpy.zeros((1, 3))
    if minEV[2] < 0:
        contactNormal[0, :] = -minEV
    else:
        contactNormal[0, :] = minEV
